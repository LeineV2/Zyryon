...
register("chat", (rank ,name) => {
    if (config.PartyCommands){
        ...
        if(!config.HideAllDebugMessages){
            ChatLib.chat(`${variables.PREFIX}&7Warped by [&b${name}&7]`)
        }
    }
...
register("chat", (name) => {
    if (config.PartyCommands){
        ...
        if(!config.HideAllDebugMessages){
            ChatLib.chat(`${variables.PREFIX}&7Toggled allinvite by [&b${name}&7]`)
        }
    }
...
register("chat", (rank, name) => {
    if (config.PartyCommands){
        ...
        if(!config.HideAllDebugMessages){
            ChatLib.chat(`${variables.PREFIX}&bPearls &7Refrashed by [&b${name}&7]`)
        }
    }
